...
void kernel_main() {
  ...
  tile_regs_acquire();
  for (int32_t i15 = v4; i15 < ((int32_t) ((uint32_t) v10 + (uint32_t) 31) / 32); i15 += v3) {
    DPRINT << "Compute loop iteration " << i15 << "\n";
    {
    ...
    DPRINT_UNPACK({
      DPRINT << "A: " << TSLICE(get_compile_time_arg_val(0), 0, SliceRange::hw0_32_16()) << ENDL();
      DPRINT << "B: " << TSLICE(get_compile_time_arg_val(1), 0, SliceRange::hw0_32_16()) << ENDL();
    });
    matmul_tiles(get_compile_time_arg_val(0), get_compile_time_arg_val(1), v1, v1, v2, v4);
