            ...
            for (const auto& core : range) {
                ...
                tt_metal::SetRuntimeArgs(
                    ...
                    {src0_dram_buffer->address(),  // Address of matrix A in DRAM
                     ...
                     K,                           
                     // strides 
                     M, // A is MxK in row major order -- triton drops the second strides
                     K, // B is KxN in row major order
                     M, // C is MxN in row major order
                     // block idx (probably need to make this work for stores)
                     0 // hard code for now 
                    });             
                ...
                tt_metal::SetRuntimeArgs(
                    ...
                    {src0_dram_buffer->address(),  // Address of matrix A in DRAM
                     ...
                     // block idx (probably need to make this work for stores)
                     0 // hard code for now 
                    });                     // Number of tiles in K dimension for dot product
...
int main() {
    ...
    try {
        ...
        constexpr uint32_t M = 32;  // Number of rows in matrix A (user-defined)
        constexpr uint32_t N = 32;  // Number of columns in matrix B (user-defined)
        constexpr uint32_t K = 32;  // Inner dimension for multiplication (user-defined)
        ...
        fmt::print("A slice:\n{}\n", slice_to_string(src0_vec, M, K));
        fmt::print("B slice:\n{}\n", slice_to_string(src1_vec, K, N));
