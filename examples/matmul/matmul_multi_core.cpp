...
void matmul_multi_core(
    ...
    fmt::print(
        "Total output tiles to compute: {} (Matrix C size: {}x{})\n", num_output_tiles_total, M, N);
    ...
    // input) that read their interleaved-DRAM layout from compile-time args.
    // Append one block per buffer *after* the CB indices, in function-input
    // order (A, B, C) -- the same order as the buffer addresses in common_args.
    // Unlike vector-add these three buffers have different shapes, so the blocks
    // genuinely differ; the kernel chains them so the runtime resolves offsets.
    ...
    auto reader_id = tt_metal::CreateKernel(
        program,
        OVERRIDE_KERNEL_PREFIX "matmul/kernels/dataflow/reader.cpp",
        all_cores,
    ...
    auto writer_id = tt_metal::CreateKernel(
        program,
        OVERRIDE_KERNEL_PREFIX "matmul/kernels/dataflow/writer.cpp",
        all_cores,
        tt_metal::DataMovementConfig{
    ...
    auto compute_kernel_id = tt_metal::CreateKernel(
        program,
        OVERRIDE_KERNEL_PREFIX "matmul/kernels/compute/matmul_triton.cpp",
        all_cores,
...
std::string slice_to_string(const std::vector<bfloat16>& mat,
                            int start, int stop, int step, int line_break_interval) {
    std::ostringstream oss;
    int line_ctr = 0;
    int crt_line = 0;
    oss << crt_line << " | ";
    for (int r = start; r < stop; r += step) {
        oss << bf16_to_float(mat[r]);
        if (r + step < stop) oss << " ";
        if (++line_ctr == line_break_interval) {
            if (r + step >= stop) break;
            oss << "\n";
            oss << ++crt_line << " | ";
            line_ctr = 0;
        }   
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Validate that dimensions are compatible with tile size
        if (M % TILE_HEIGHT != 0 || N % TILE_WIDTH != 0 || K % TILE_WIDTH != 0) {
            TT_THROW("Matrix dimensions must be divisible by tile size (M % {} == 0, N % {} == 0, K % {} == 0)", 
        ...
        fmt::print("B post-tilize:\n{}\n", slice_to_string(src1_vec, /*start=*/0, /*stop=*/K * N, /*step=*/16, /*line_break_every=*/N / 16));
#endif 
