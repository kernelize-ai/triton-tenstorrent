 ...
 * @param bias Bias matrix to be added to the result (bfloat16 elements)
...
void matmul_multi_core(
    ...
    uint32_t BN = 32;  // block N in elements
    uint32_t BK = 32;  // block K in elements
    ...
    auto bias_dram_buffer = distributed::MeshBuffer::create(buffer_config_C, dram_config, mesh_device.get());
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::vector<bfloat16> bias_vec(M * N, 0);  // Bias Matrix (MxN)
        ...
        matmul_multi_core(src0_vec, src1_vec, bias_vec, result_vec, M, N, K, mesh_device);
