...
//
// Identical algorithm to ../../../histc/kernels/compute/histc_compute.cpp
// (compare-and-reduce, one bin at a time -- see that file's header comment
// for the two-phase DST-register structure). The only difference is scope:
// `num_tiles` here is this core's *shard* of the input (tiles_per_core from
// histc_hybrid.cpp), not the whole tensor -- every core in the range runs
// this same compiled kernel over its own slice, in parallel.
...
void MAIN {
    ...
    const uint32_t num_tiles = get_compile_time_arg_val(4);  // this core's shard, not the whole tensor
    ...
    for (uint32_t b = 0; b < bins; ++b) {
        ...
        // ---- Phase B: count[b] = sum over all of this shard's mask tiles ----
