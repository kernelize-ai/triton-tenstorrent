...
torch.manual_seed(0)
size = 66560
x = torch.rand(size, device=DEVICE).to(torch.float32)
y = torch.rand(size, device=DEVICE).to(torch.float32)
...
# TMP: Don't run until we have a tenstorrent driver
#benchmark.run(print_data=True, show_plots=True, save_path='/tmp/vector_add/')
