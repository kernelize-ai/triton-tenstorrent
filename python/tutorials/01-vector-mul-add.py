...
In this tutorial, you will write a simple vector multiplication using Triton.
...
@triton.jit
def muladd_kernel(x_ptr,  # *Pointer* to first input vector.
...
def muladd(x, y, z):
    ...
    muladd_kernel[grid](x, y, z, output, n_elements, BLOCK_SIZE=BLOCK_SIZE, num_warps=1)
...
z = torch.rand(size, device=DEVICE, dtype=torch.bfloat16)
output_torch = (x * y) + z
...
output_triton_dev = muladd(dev_x, dev_y, dev_z)
output_triton = torch.zeros_like(output_torch)
triton.runtime.driver.active.copy_buffer_to_host(output_triton_dev, output_triton)
print(output_torch)
print(output_triton)
...
benchmark.run(print_data=True, show_plots=True, save_path='/tmp/01-mul-vectors')
