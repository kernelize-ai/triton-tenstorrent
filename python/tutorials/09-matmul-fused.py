...
def matmul_tma_set_block_size_hook(nargs):
    ...
    nargs["bias_desc"].block_shape = nargs["c_desc"].block_shape
...
def matmul_kernel_fused(a_desc, b_desc, c_desc, bias_desc,  #
    ...
    # fused add bias
    bias = bias_desc.load([offs_cm, offs_cn])
    c = c + bias

    c_desc.store([offs_cm, offs_cn], c)


def matmul_tma(a, b, bias, warp_specialize: bool):
    # Check constraints.
    ...
    assert bias.shape == (M, N), "Incompatible bias dimensions"
    ...
    bias_desc = TensorDescriptor.from_tensor(bias, dummy_block)
...
def bench(K, dtype, reps=10000, warmup_reps=10000):
    ...
    bias = torch.randn((M, N), device="cuda", dtype=torch.float16).to(dtype)
    ...
    for ws in warp_specialize:
        ...
        if HAS_HOST_TENSOR_DESC and not (is_hopper() and ws):
            bench_fn(f"tma{ws_str}", reps, warmup_reps, lambda a, b: matmul_tma(a, b, bias, ws), a, b)


def run_test(expect, fn, a, b, bias, label, enabled=True):
    print(f"  {label}: ...", end="")
    if enabled:
        actual = fn(a, b, bias)
...
def validate(M, N, K, dtype):
    ...
    bias = torch.randn((M, N), device="cpu", dtype=torch.float16).to(dtype)
    #b = b.T.contiguous()

    naive_result = torch_matmul(a, b.T).to(torch.float16)
    triton_result = matmul_tma(a, b, bias, False)
    print(f"triton_result: {triton_result}")
    print(f"naive_result: {naive_result}")
    """
    run_test(naive_result, torch_matmul, a, b, bias, "Torch", enabled=dtype == torch.float16)
