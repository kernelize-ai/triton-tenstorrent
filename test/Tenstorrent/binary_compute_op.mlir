...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "cpu", "ttg.threads-per-warp" = 1 : i32} {
    // CHECK: @max_kernel
    tt.func public @max_kernel(%x_ptr: !tt.ptr<f32> {tt.divisibility = 8 : i32}, %y_ptr: !tt.ptr<f32> {tt.divisibility = 8 : i32}, %output_ptr: !tt.ptr<f32> {tt.divisibility = 8 : i32}, %n_elements: i32 {tt.divisibility = 8 : i32}) attributes {noinline = false} {
        ...
        // CHECK: %[[Y:.*]] = tt.load
        %y = tt.load %y_ptrs_offset : tensor<1024x!tt.ptr<f32>, #blocked>
        // CHECK: triton_tenstorrent.binary_compute["arith.maximumf"] %[[X]], %[[Y]] : (tensor<1024xf32, #blocked>, tensor<1024xf32, #blocked>) -> tensor<1024xf32, #blocked>
        %output = arith.maximumf %x, %y : tensor<1024xf32, #blocked>
        %output_ptrs = tt.splat %output_ptr : !tt.ptr<f32> -> tensor<1024x!tt.ptr<f32>, #blocked>
        %output_ptrs_offset = tt.addptr %output_ptrs, %offsets : tensor<1024x!tt.ptr<f32>, #blocked>, tensor<1024xi32, #blocked>
        // CHECK: tt.store
        tt.store %output_ptrs_offset, %output : tensor<1024x!tt.ptr<f32>, #blocked>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "cpu", "ttg.threads-per-warp" = 1 : i32} {
    // CHECK: @min_kernel
    tt.func public @min_kernel(%x_ptr: !tt.ptr<f32> {tt.divisibility = 8 : i32}, %y_ptr: !tt.ptr<f32> {tt.divisibility = 8 : i32}, %output_ptr: !tt.ptr<f32> {tt.divisibility = 8 : i32}, %n_elements: i32 {tt.divisibility = 8 : i32}) attributes {noinline = false} {
        ...
        // CHECK: %[[Y:.*]] = tt.load
        %y = tt.load %y_ptrs_offset : tensor<1024x!tt.ptr<f32>, #blocked>
        // CHECK: triton_tenstorrent.binary_compute["arith.minimumf"] %[[X]], %[[Y]] : (tensor<1024xf32, #blocked>, tensor<1024xf32, #blocked>) -> tensor<1024xf32, #blocked>
        %output = arith.minimumf %x, %y : tensor<1024xf32, #blocked>
        %output_ptrs = tt.splat %output_ptr : !tt.ptr<f32> -> tensor<1024x!tt.ptr<f32>, #blocked>
        %output_ptrs_offset = tt.addptr %output_ptrs, %offsets : tensor<1024x!tt.ptr<f32>, #blocked>, tensor<1024xi32, #blocked>
        // CHECK: tt.store
        tt.store %output_ptrs_offset, %output : tensor<1024x!tt.ptr<f32>, #blocked>
